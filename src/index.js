// 기능요구사항
// (ㅇ) 랜덤 숫자 3개 생성(서로 달라야함)
// (ㅇ) 사용자가 입력한 서로 다른 3개의 값을 랜덤 숫자 3개와 비교해 스트라이크, 볼, 낫싱 출력해야함(볼이 스트라이크보다 앞에 출력)
// () 사용자가 모두 맞히면(=3스트라이크) 게임이 종료되고, 재시작 버튼이 노출된다.
// () 재시작 버튼을 통해 다시 시작할 수 있다.
// (ㅇ) 사용자가 잘못된 값을 입력하면 alert로 에러 메시지를 보여주고 다시 입력

//프로그래밍 요구사항
// (ㅇ) play메서드는 string으로 리턴
// () 게임을 다시 시작하는 버튼 태그의 id는 game-restart-button으로 생성
// (ㅇ) Math.random대신 MissionUtils 라이브러리의 랜덤 사용
// () 순수 js로만 구현
// () 코드 컨벤션 지키기
